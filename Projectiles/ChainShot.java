 ...
 */
public class ChainShot extends Projectiles{
	
	public ChainShot(Screen screen, int ref, TimeManager currentTime, BasicShape shape, Point shooterPosition) {
		super(screen, ref, currentTime, shape, shooterPosition);
		id = 4;
		name = "chainshot";
		shapeChangingDist = 42 + ( 16*getCurrentTime().getHour() + 4*getCurrentTime().getMinute() + getCurrentTime().getSecond()) % 42;
	...
	public void shoot(int dist, Point shooterPosition) {
		this.shooterPosition = shooterPosition.translate(0, -dist);
	}
	
	public void dilatation(int dist, int ref) {
		this.ref = ref - dist/10 - 4;
	}
