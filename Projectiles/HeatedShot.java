 ...
 */
public class HeatedShot extends Projectiles{ 
	
	public HeatedShot(Screen screen, int ref, TimeManager currentTime, BasicShape basicShape, Point shooterPosition) {
		super(screen, ref, currentTime, basicShape, shooterPosition);
		id = 6;
		name = "heatedshot";
		shapeChangingDist = 42 + ( 36*getCurrentTime().getHour() + 6*getCurrentTime().getMinute() + getCurrentTime().getSecond()) % 42;
	...
	public void shoot(int dist, Point shooterPosition) {
		this.shooterPosition = shooterPosition.translate( 0, (int) Math.round(Math.cos(dist*Math.PI/2)));
	}
	
	public void dilatation(int dist, int ref) {
		this.ref = ref - dist/10 - 6;
	}
